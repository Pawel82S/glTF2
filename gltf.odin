...
accessor_sparse_parse :: proc(object: json.Object) -> (res: Accessor_Sparse, err: Error) {
    ...
    for k, v in object {
        ...
        case "indices":
            // Required
            res.indices = sparse_indices_parse(v.(json.Object)) or_return
            indices_set = true

        case "values":
            // Required
            res.values = sparse_values_parse(v.(json.Object)) or_return
            values_set = true
...
@(require_results)
sparse_indices_parse :: proc(object: json.Object) -> (res: Accessor_Sparse_Indices, err: Error) {
    ...
    for k, v in object {
        ...
        case "bufferView":
            // Required
            res.buffer_view = Integer(v.(f64))
            buffer_view_set = true

        case "byteOffset":
            // Default 0
            res.byte_offset = Integer(v.(f64))

        case "componentType":
            // Required
            res.component_type = Component_Type(v.(f64))
            component_type_set = true
...
@(require_results)
sparse_values_parse :: proc(object: json.Object) -> (res: Accessor_Sparse_Values, err: Error) {
